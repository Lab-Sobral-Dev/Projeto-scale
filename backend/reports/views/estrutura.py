...
from registro.models import EstruturaProduto, ItemEstrutura, Produto
...
class EstruturaProdutoReportView(APIView):
    ...
    def get(self, request):
        ...
        # status_mp: 'true'/'false' → filtra MPs por ativo
        ...
        for e in qs:
            for it in e.itens.select_related('materia_prima').all():
                if status_mp in ('true','false'):
                    if it.materia_prima.ativo != (status_mp=='true'):
                        continue
                rows.append([e.produto.nome, e.descricao, it.materia_prima.nome, f"{it.quantidade_por_lote:.3f}", "Sim" if it.materia_prima.ativo else "Não"])
                data.append({
