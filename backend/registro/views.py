...
class RequireDeleteReasonAuditMixin:
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if motivo not in self.DELETE_MOTIVOS:
            return Response({"detail": "Motivo inválido. Use um dos motivos padrões."}, status=status.HTTP_400_BAD_REQUEST)
...
class EstruturaProdutoViewSet(RequireDeleteReasonAuditMixin, viewsets.ModelViewSet):
    ...
    PROTECTED_MESSAGE = "Não é possível excluir: existem registros vinculados (ex.: OPs geradas a partir desta estrutura)."
    DELETE_MOTIVOS = {
        "cadastro_duplicado": "Cadastro duplicado",
