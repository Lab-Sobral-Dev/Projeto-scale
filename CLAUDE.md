...
O fluxo de trabalho deste repo (Projeto-scale) é **GitHub Issues**. TUDO vira issue —
ideias, pendências, tasks, bugs, decisões técnicas. Issue aberta = em progresso;
issue fechada (com ref ao commit) = concluído.

Como abrir:
```bash
gh issue create --title "[ÁREA] título curto" --label <task|feature|improvement|bug|discussion> \
  --body $'**O quê:** ...\n**Por quê:** ...\n**Done:** <critério verificável>'
```

Ao concluir: `gh issue close <n> --comment "feito em <commit/PR>"`.
Proibido: trabalho sem issue; issue vaga sem o quê/por quê/done.
